...
class GAN():
    ...
    def train(self, x_train, batch_size, epochs, run_folder, print_every_n_batches = 50, initial_epoch = 0):
        ...
        for epoch in range(initial_epoch, initial_epoch + epochs):
            ...
            # d_loss_real, d_acc_real  = self.discriminator.train_on_batch(true_imgs, np.ones((batch_size,1)))
            # d_loss_fake, d_acc_fake = self.discriminator.train_on_batch(gen_imgs, np.zeros((batch_size,1)))
            ...
            # Plot the progress
            print ("%d [D loss: %f] [D acc: %f] [G loss: %f] [G acc: %f]" % (epoch, d_loss, d_acc, g_loss, g_acc))
